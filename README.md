...
| 파일 | 역할 |
|---|---|
| `index.html` | **제안용 시세판** — 공급가는 "별도 문의"로 가려짐. 신규 거래 제안 시 이 링크를 보냅니다 |
| `partners.html` | **거래처용 시세판** — 공급가가 항상 그대로 보임. 이미 거래 중인 업체에게만 이 링크를 보냅니다 |
| `vendor.html` | 상품 등록 폼 — 창고/공급가 입력 → 실시간 카드 미리보기 → 표·TSV·JSON 내보내기 |
| `format.html` | 데이터 제출 형식 안내 (거래처 공급사용) |
| `data/products.json` | 실제 표시되는 상품 데이터 — 두 시세판이 공유합니다. 이 파일만 갈아끼우면 양쪽 다 바뀝니다 (전체 취급 품목 647종) |
